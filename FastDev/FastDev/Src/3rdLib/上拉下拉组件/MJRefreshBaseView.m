...
- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        ...
        if (self.viewType == MJRefreshViewTypeFooter) {
            // 3.状态标签
            [self addSubview:_statusLabel = [self labelWithFontSize:15]];
            /*
            // 4.箭头图片
            UIImageView *arrowImage = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"icon_shifang@2x.png"]];
        ...
        if (self.viewType == MJRefreshViewTypeHeader) {
            // 3.状态标签
            [self addSubview:_statusLabel = [self labelWithFontSize:15]];
            // 4.箭头图片
            UIImageView *jtImageView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"icon_shifang@2x.png"]];
            jtImageView.autoresizingMask = UIViewAutoresizingFlexibleLeftMargin | UIViewAutoresizingFlexibleRightMargin;
            [self addSubview:_customView = jtImageView];
            //三妹图片
            UIImageView *arrowImage = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, 50, 60)];
            arrowImage.animationImages = [NSArray arrayWithObjects:
                                          [Tools imagesNamedFromCustomBundle:@"三妹1" bundleName:TABBAR_BUNDLE],
                                          [Tools imagesNamedFromCustomBundle:@"三妹2" bundleName:TABBAR_BUNDLE], nil];
            [arrowImage setAnimationDuration:1.0f];
            [arrowImage setAnimationRepeatCount:0];
            [arrowImage startAnimating];
            arrowImage.autoresizingMask = UIViewAutoresizingFlexibleLeftMargin | UIViewAutoresizingFlexibleRightMargin;
            [self addSubview:_arrowImage = arrowImage];
            /*
             // 5.指示器
...
- (void)setFrame:(CGRect)frame
{
    ...
    // 1.状态标签
    _statusLabel.frame = CGRectMake(statusX + 12, statusY - 5, statusWidth, statusHeight);
    
    // 2.时间标签
    CGFloat lastUpdateY = statusY + statusHeight + 5;
    _lastUpdateTimeLabel.frame = CGRectMake(statusX + 6, lastUpdateY - 8, statusWidth, statusHeight);
    
    // 3.箭头
    CGFloat customX = w * 0.5 - 55;
    _customView.center = CGPointMake(customX + 12, h * 0.5 - 8);
    //4.三妹
    if (self.viewType == MJRefreshViewTypeHeader) {
        CGFloat arrowX = w * 0.5 - 120;
        _arrowImage.center = CGPointMake(arrowX, h * 0.5
                                         );
    }
    /*
    if (self.viewType == MJRefreshViewTypeFooter) {
        _arrowImage.center = CGPointMake(customX - 10, h * 0.5 - 3);
    }
     */
    // 5.指示器
